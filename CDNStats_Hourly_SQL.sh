...
echo " Checking under process... "
		...
		if [ $ZONE = 'EMS' ]; then
			FILTER="awk -v HN=$HOST_NAME -v IP=$IPADR -v APP_ID=$DPID -v Z=$ZONE 'BEGIN{FS=\"|\";OFS=\",\";}{tot[substr(\$4,2,16)]++;if(\$1 ~ /[0-9][0-9][0-9][0-9]:/) ipv6[substr(\$4,2,16)]++; else ipv4[substr(\$4,2,16)]++; if (\$7 ~ /2[0-9][0-9]/) success[substr(\$4,2,16)]++; else failure[substr(\$4,2,16)]++; data[substr(\$4,2,16)]+=\$8; time[substr(\$4,2,16)]+=\$11; if (\$11>$SEC) buff[substr(\$4,2,16)]++; if (\$5 ~ /HIT/) hit[substr(\$4,2,16)]++; else miss[substr(\$4,2,16)]++;} END {for (var in tot) print \"STR_TO_DATE( \x27\" substr(var,1,11) \"\x27\", \"\x27\" \"%d/%b/%Y\" \"\x27 )\",  \"\42\" substr(var,13,16) \"\42\", APP_ID, \"\42\" Z \"\42\", \"\42\" HN \"\42\", \"\42\" IP \"\42\", buff[var]+1,  tot[var]+2, buff[var]*100/(tot[var]+.001),  hit[var]+1, miss[var]+1, hit[var]*100/(tot[var]+.0001), success[var]+1, failure[var]+1, success[var]*100/(tot[var]+.001), time[var]/(tot[var]+.0001), data[var] ,  ipv6[var]+1, ipv4[var]+1, ipv6[var]*100/(tot[var]+.0001), \"VISHAL\";}'"
		else
			FILTER="awk -v HN=$HOST_NAME -v IP=$IPADR -v APP_ID=$DPID -v Z=$ZONE 'BEGIN{FS=\"|\";OFS=\",\";}{tot[substr(\$4,2,16)]++;if(\$2 ~ /[0-9][0-9][0-9][0-9]:/) ipv6[substr(\$4,2,16)]++; else ipv4[substr(\$4,2,16)]++; if (\$7 ~ /2[0-9][0-9]/) success[substr(\$4,2,16)]++; else failure[substr(\$4,2,16)]++; data[substr(\$4,2,16)]+=\$8; time[substr(\$4,2,16)]+=\$11; if (\$11>$SEC) buff[substr(\$4,2,16)]++; if (\$5 ~ /HIT/) hit[substr(\$4,2,16)]++; else miss[substr(\$4,2,16)]++;} END {for (var in tot) print \"STR_TO_DATE( \x27\" substr(var,1,11) \"\x27\", \"\x27\" \"%d/%b/%Y\" \"\x27 )\",  \"\42\" substr(var,13,16) \"\42\", APP_ID, \"\42\" Z \"\42\", \"\42\" HN \"\42\", \"\42\" IP \"\42\", buff[var]+1,  tot[var]+2, buff[var]*100/(tot[var]+.001),  hit[var]+1, miss[var]+1, hit[var]*100/(tot[var]+.0001), success[var]+1, failure[var]+1, success[var]*100/(tot[var]+.001), time[var]/(tot[var]+.0001), data[var] ,  ipv6[var]+1, ipv4[var]+1, ipv6[var]*100/(tot[var]+.0001), \"VISHAL\";}'"
		fi
#if [ $DAY = 'Y' ]; then
