...
if [ $ZONE = 'EMS' ]; then
BUFFER="awk 'BEGIN {FS=\"|\";OFS=\",\";i=0;j=0;k=0;l=0;x=0;y=0;tt=0;td=0;} {j=j+1; if(\$1 ~ /[0-9][0-9][0-9][0-9]:/) ips=ips+1; else ipf=ipf+1; if (\$6 ~ /2[0-9][0-9]/) x=x+1; else y=y+1; td=td+\$7; tt=tt+\$10; if (\$10>$SEC) i=i+1; if (\$4 ~ /HIT/) k=k+1; else l=l+1;} END {print i,j,i*100/(j+.0001),k,l,k*100/(k+l+.0001),x,y,x*100/(x+y+.0001),tt/(j+.0001),td/1024/1024/1024}'"
else
BUFFER="awk 'BEGIN {FS=\"|\";OFS=\",\";i=0;j=0;k=0;l=0;x=0;y=0;tt=0;td=0;} {j=j+1; if(\$2 ~ /[0-9][0-9][0-9][0-9]:/) ips=ips+1; else ipf=ipf+1; if (\$7 ~ /2[0-9][0-9]/) x=x+1; else y=y+1; td=td+\$8; tt=tt+\$11; if (\$11>$SEC) i=i+1; if (\$5 ~ /HIT/) k=k+1; else l=l+1;} END {print i,j,i*100/(j+.0001),k,l,k*100/(k+l+.0001),x,y,x*100/(x+y+.0001),tt/(j+.0001),td/1024/1024/1024}'"
fi
