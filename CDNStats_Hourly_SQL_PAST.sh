...
DB_PORT="3306"
DB_DBNAME="sasodb"
rm /tmp/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$PAST_DATE.sql
rm /home/jioadm/reports/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$PAST_DATE
#cat /dev/null > /tmp/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$DATE.sql
if [ $# -lt 5 ]; then
echo "Format:  sh FetchCDNStats_Day.sh DS/EMS all DPID >SECONDS PAST_DATE(20170316)"
...
#		DATA1=$(sshpass -p $PASS $SSH $USR@$IPADR "cat /var/log/nginx/access_$DPID\.log-$DATE /var/log/nginx/access_$DPID\.log | grep -v ' 503 '| $FILTER") #>>/home/jioadm/reports/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$DATE
		DATA1=$(sshpass -p $PASS $SSH $USR@$IPADR "zcat /var/log/nginx/access_$DPID\.log-$PAST_DATE\.gz | $FILTER") #>>/home/jioadm/reports/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$DATE
#fi
#$(sshpass -p $PASS $SSH $USR@$IPADR "cat /var/log/nginx/access_$DPID\.log-$DATE /var/log/nginx/access_$DPID\.log | grep -v ' 503 '| $FILTER") >>/home/jioadm/reports/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$DATE
		if [ $? -eq 0 ]; then
			echo $DATA1 >>/home/jioadm/reports/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$PAST_DATE
		#else
...
sed -i "s/,VISHAL/);\\n/g" /home/jioadm/reports/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$PAST_DATE #> /home/jioadm/reports/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$DATE.csv
sed -i "/^\s*$/d" /home/jioadm/reports/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$PAST_DATE
sed -i "s///g" /home/jioadm/reports/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$PAST_DATE
sed -e "s/^/INSERT IGNORE INTO SASO_CDN_HOURLY_STATS(DATE,HOUR_MIN,APP_ID,ZONE,HOSTNAME,IP,BUFFERING_REQUESTS,TOTAL_REQUESTS,BUFFERING_PERCENTAGE,TOTAL_HITS,TOTAL_MISS,HIT_PERCENTAGE,TOATAL_SUCCESS,TOTAL_FAILURES,SUCCESS_PERCENTAGE,AVG_RESPONSE_TIME,DATA_CONSUMPTION_IN_GB,IPV6_COUNT,IPV4_COUNT,IPV6_PERCENTAGE) VALUES(/g" /home/jioadm/reports/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$PAST_DATE > /tmp/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$PAST_DATE.sql
...
checkDbConnection
echo "mysql -h $DB_HOST -u $DB_USER -p$DB_USER_PASSWORD -P $DB_PORT $DB_DBNAME < /tmp/CDN_HOURLY_STATS_$ZONE-$SITENAME-$DPID-$PAST_DATE.sql"
